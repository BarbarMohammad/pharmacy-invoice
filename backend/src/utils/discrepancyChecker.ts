export function checkDiscrepancies(invoiceRecords: any[], refDrugs: any[]) {
  const discrepancies = [];
  for (let i = 1; i < invoiceRecords.length; i++) {
    ...
    const ref = refDrugs.find((d: any) =>
      d.drugName.trim().toLowerCase() === String(drugName).trim().toLowerCase()
    );
    if (!ref) continue;
